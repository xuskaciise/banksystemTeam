...
let FileCv=document.querySelector("#cv");
let Showcv=document.querySelector("#showCv");
...
$("#employeeForm").on("submit", function(event){
    event.preventDefault();
     formdata=new FormData($("#employeeForm")[0]);
     formdata.append("image",$("input[type=file")[0].files[0]);
     formdata.append("cv",$("input[type=file")[1].files[0]);
            ...
            if(status){
                response.forEach(function(res){
                    ...
                    for(let r in res){
                        ...
                        if(r=="image" ){
                         html+=`<td><img src='../uploads/employee/images/${res[r]}' style='width:50px;height:50px;border-radius:50%;object-fit:cover;margin-left:20px' ></td>`
                         html+=`<td><img src='../uploads/employee/cv/${res[r]}' style='width:50px;height:50px;border-radius:50%;object-fit:cover;margin-left:20px' ></td>`
                        }
                        // }else if(r== "cv"){
                        //     html+=`<td><img src='../uploads/employee/cv/${res[r]}' style='width:50px;height:50px;border-radius:50%;object-fit:cover;' ></td>`
                        // }
                        else{
                            html+=`<td>${res[r]}</td>`;
                        }
      ...
      $.ajax({
          ...
          success:function(data){
              ...
              if(status){
                ...
                $("#showImage1").attr('src',`../uploads/employee/images/${response['image']}`)
                $("#showCv1").attr('src',`../uploads/employee/cv/${response['cv']}`)
                // $("#date").html(response['date']);
...
$(".employeeTable tbody").on("click","a.delete_info", function(){
...
$(".employeeTable tbody").on("click","a.update_info", function(){
    let id=$(this).attr("update_id");
    FeachData(id);

})
$(".employeeTable tbody").on("click","a.see_info", function(){
